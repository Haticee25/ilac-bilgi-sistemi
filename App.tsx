...
// Klasör yapına göre hooks yollarını güncelledim
import { useCamera } from './components/hooks/useCamera';
import { useOCR } from './components/hooks/userOCR';
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch("/liste.csv")
      ...
      .then(csvData => {
        Papa.parse(csvData, {
          header: true,
          delimiter: ";", // Senin dosyan noktalı virgül kullandığı için kritik
          skipEmptyLines: true,
          complete: (results: any) => {
            // Sadece "İlaç Adı" dolu olan satırları filtreliyoruz
  ...
  const handleSearch = (name: string) => {
    ...
    else alert("İlaç veritabanında bulunamadı!");
  };
  ...
  const handleCapture = async () => {
    ...
    if (image) {
      ...
      try {
        ...
        if (text) {
          // Okunan metin içinde listedeki ilaçlardan biri geçiyor mu?
          const found = drugs.find(d => 
  ...
  return (
    <div style={{ 
      ...
      {/* İlaç yüklenme durumu kontrolü */}
      <p style={{ color: '#93c5fd' }}>
        {drugs.length > 0 ? `${drugs.length} İlaç Veritabanı Hazır` : "Veriler yükleniyor..."}
      </p>
      ...
      {!showCamera ? (
        <div style={{ maxWidth: '400px', margin: '0 auto' }}>
          <div style={{ position: 'relative', marginBottom: '30px' }}>
            ...
            />
            <button onClick={() => handleSearch(query)} style={{ position: 'absolute', right: '5px', top: '5px', padding: '8px 15px', borderRadius: '20px', border: 'none', background: '#38bdf8', fontWeight: 'bold' }}>Ara</button>
          </div>

          <button 
            onClick={() => { setShowCamera(true); setTimeout(startCamera, 500); }}
            style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#22c55e', border: 'none', fontSize: '30px', cursor: 'pointer', boxShadow: '0 0 20px rgba(34,197,94,0.4)' }}
          >📷</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'black', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ height: '100px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px' }}>
            <button onClick={handleCapture} style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'white', border: 'none' }} />
            <button onClick={() => { stopCamera(); setShowCamera(false); }} style={{ color: 'white', background: 'none', border: '1px solid white', padding: '10px 20px', borderRadius: '20px' }}>İptal</button>
          </div>
        </div>
      )}
      ...
      {result && !showCamera && (
        <div style={{ marginTop: '30px', padding: '20px', borderRadius: '15px', background: 'rgba(255,255,255,0.1)', textAlign: 'left', border: '1px solid rgba(255,255,255,0.2)' }}>
          ...
          <p><strong>⚠️ Yan Etkiler:</strong> {result[" Yan Etkiler"]}</p>
          <p><strong>🔞 Yaş Aralığı:</strong> {result["Yaş Aralığı"]}</p>
          <button onClick={() => setResult(null)} style={{ background: 'none', border: 'none', color: '#93c5fd', textDecoration: 'underline', cursor: 'pointer', padding: 0 }}>Temizle</button>
        </div>
      )}
    </div>
  );
